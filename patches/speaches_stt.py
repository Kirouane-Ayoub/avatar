    ...
    if whisper_utils.hf_model_filter.passes_filter(model_card_data):
        with model_manager.load_model(model) as whisper:
            ...
            segments, transcription_info = whisper_model.transcribe(
                ...
                vad_filter=effective_vad_filter,
                hotwords=hotwords,
                # ── Anti-hallucination knobs (avatar patch) ──
                # Whisper happily fabricates phrases on silence/noise and
                # loops them when conditioned on its own output. These
                # thresholds raise the confidence bar so phantom turns get
                # dropped before reaching the LLM.
                #
                # condition_on_previous_text=False: stop "彼らは、彼らは…"
                #   style loops at the source.
                # log_prob_threshold=-0.5 (default -1.0): only confident
                #   segments pass — raises confidence floor.
                # no_speech_threshold=0.5 (default 0.6): more aggressive
                #   about declaring silence (drops more empty segments).
                # compression_ratio_threshold=2.2 (default 2.4): tighter
                #   loop-detector — drops repetitive output earlier.
                # repetition_penalty=1.2 (default 1.0): decode-time
                #   discouragement of token loops.
